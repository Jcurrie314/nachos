...
public class VMKernel extends UserKernel {
	...
	int numPhysPages = Machine.processor().getNumPhysPage();
	coreMap = new pageFrame[numPhysPages];
	for( int i = 0; i < numPhysPages; i++ )
	    coreMap = new pageFrame();
    }
	...
	for( int i = 0; i < Machine.processor().getTLBSize(); i++ )
		...
		if( TLBEntry.valid )
			...
			//true
			if( contextSwitch )  TLBEntry.valid = false;
	...
	private LinkedList<Integer> PageTableIDs = new LinedList<int>();
	private LinkedList<Integer> unusedFileSpace = new LinkedList<int>();
	...
	{
	    int spn = 0;
	    if( usnusedFileSpace != null ) spn = unusedFileSpace.pop();
	    else spn = PageTableIDs.getLast() + 1;
	    
	    swapf.write( spn * pageSize, coreMap, ppn * pageSize, pageSize );
